...
PF_BufferMgr::PF_BufferMgr(int _numPages) : hashTable(PF_HASH_TBL_SIZE) {
...
#ifdef PF_STATS
    // Initialize the local variable for the statistics manager
    ...
    // Allocate memory for buffer page description table
    // Initially, the freee list contains all pages.
    for (int i = 0; i < numPages; ++i) {
        if((bufTable[i].pData = new char[pageSize]) == NULL) {
            cerr << "Not enough memroy for buffer\n";
            exit(1);
        }
...
#ifdef PF_LOG
    WriteLog("Successfully created the new buffer manager.\n");
#endif
}
...
PF_BufferMgr::~PF_BufferMgr() {
    // Free up buffer pages and tbles
    for (int i = 0; i < numPages; ++i) {
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Allocating a page for (%d,%d)....\n", fd, pageNum);
    WriteLog(psMessage);
#endif
...
RC PF_BufferMgr::MarkDirty(int fd, PageNum pageNum) {
    ...
    // Make this page as the most recently used page
    if ((rc = Unlink(slot)) || (rc = LinkHead(slot)))
        return (rc);
...
#ifdef PF_LOG
    char psMessage[100];
    sprintf(psMessage, "Unpining (%d,%d).\n", fd, pageNum);
#endif
...
RC PF_BufferMgr::FlushPages(int fd) {
...
#ifdef PF_STAT
    pStatistics->Register(PF_FLUSHPAGE, STAT_ADDONE);
#endif
    ...
    WriteLog(psMessage);
                ...
                // Remove page from the hash table and add the slot to the
                // free list
...
RC PF_BufferMgr::ResizeBuffer(int iNewSize) {
    ...
    pNewBufTable[0].prev = INVALID_SLOT;
    pNewBufTable[iNewSize - 1].next = INVALID_SLOT;
...
RC PF_BufferMgr::Unlink(int slot) {
    ...
    // If slot not at the end of list, point next back to previous
    if (bufTable[slot].next != INVALID_SLOT) {
        bufTable[bufTable[slot].next].prev = bufTable[slot].prev;
    }

    // If slot not at the head of list, point prev forward to previous
...
RC PF_BufferMgr::ReadPage(int fd, PageNum pageNum, char* dest) {
    ...
    // seek to the targeting place for this page
...
// Ret:  PF return code
//
RC PF_BufferMgr::WritePage(int fd, PageNum pageNum, char* src) {
#ifdef PF_LOG
    char psMessage[100];
